...
class Sampler(nn.Module):
    ...
    def __init__(
        self, vocab_size: int, world_size: int, rank: int, config: GemmaConfig
    ) -> None:
        super().__init__()
    ...
    def forward(
        ...
        hidden_states_parallel = scatter_to_model_parallel_region(
            hidden_states, groups=None, world_size=self.world_size, rank=self.rank
        )
...
class GemmaModel(nn.Module):
    ...
    def __init__(self, config: GemmaConfig, world_size: int, rank: int):
        super().__init__()
        self.config = config
        self.rank = rank
        ...
        for i in range(config.num_hidden_layers):
            self.layers.append(Gemma2DecoderLayer(config, world_size, rank))
    ...
    def _load_weights(self, model_state_dict: Mapping[str, torch.Tensor]):
        ...
        for k, v in model_state_dict.items():
            ...
            if (
                ...
                r"model.layers.\d+.mlp.gate_proj.weight", k
            ) or re.fullmatch(r"model.layers.\d+.mlp.up_proj.weight", k):
                v = split(v, 0)
            elif re.fullmatch(r"model.layers.\d+.self_attn.qkv_proj.weight", k):
                if num_kv_heads <= num_attn_heads:
                    ...
                    v = v.reshape(
                        num_attn_heads + num_kv_heads * 2, head_dim, hidden_size
                    )
                    query = v[:num_attn_heads, ...]
                    key = v[num_attn_heads : num_attn_heads + num_kv_heads, ...].repeat(
                        num_replicas, 1, 1
                    )
                    value = v[-num_kv_heads:, ...].repeat(num_replicas, 1, 1)
                    v = torch.cat(
                        (split(query, 0), split(key, 0), split(value, 0)), dim=0
                    )
                else:
